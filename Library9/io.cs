...
public class MyIO : IDisposable
{
    ...
    public (TQuery Type, string Content) ReadQuery<TQuery>() where TQuery : ISpanParsable<TQuery> 
        => reader.ReadLine()!.SplitAs<TQuery, string>();
    ...
    private List<T> Split<T>(string s, int capacity = -1) where T : ISpanParsable<T>
    {
        ...
        {
            result.Add(T.Parse(chars[range], null));
        }
        return result;
    }
    ...
    private T[] SplitArray<T>(string s, int capacity) where T : ISpanParsable<T>
    {
        ...
        foreach(var range in chars.Split(' '))
        {
            result[index++] = T.Parse(chars[range], null);
        }
        return result;
    }
    ...
    public void WriteLine(IEnumerable<double> list) => writer.WriteLine(string.Join(' ', list));
